                 ...
                 <el-row>
                     <el-col :span="10">
                         <el-form-item label="开票日期：" prop="invoiceTimeGap">
                             <el-date-picker value-format="yyyy-MM-dd" v-model="queryFormData.invoiceTimeGap" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" ></el-date-picker>
                         </el-form-item>
                     </el-col>
                     <el-col :span="5">
...
export default {
  ...
  data() {
    var dateGapValidate = (rule, value, callback) => {
      ...
      let startTime = new Date(value[0].replace(/-/g, "/")).getTime();
      let endTime = new Date(value[1].replace(/-/g, "/")).getTime();
